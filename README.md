...
This README describes the current `main` branch. The latest tagged release is `v0.9.3`.
...
Uses the prebuilt image from GHCR pinned to the latest tagged release by default (`ghcr.io/ovumcy/ovumcy-web:v0.9.3`).
...
```bash
OVUMCY_IMAGE=ghcr.io/ovumcy/ovumcy-web:v0.9.3 docker compose up -d
```
