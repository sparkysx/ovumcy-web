...
| Art. 17 — Right to erasure | `clear-data` removes health records; `delete-account` removes the entire account including cascaded OIDC identities | `POST /api/v1/users/current/data-wipe`, `DELETE /api/v1/users/current` | `internal/api/settings_clear_data_flow_test.go`, `internal/api/settings_delete_account_regression_test.go`, `TestClearDataPreservesAccountIdentityFields` |
| Art. 20 — Right to portability | Same export endpoints return machine-readable formats; structure documented in `docs/export.md` | `GET /api/v1/exports/csv`, `GET /api/v1/exports/json` | `internal/api/export_regressions_test.go`, `internal/services/export_service_integration_test.go` |
