...
This README describes the current `main` branch. The latest tagged release is `v1.5.0`.
...
Uses the prebuilt image from GHCR pinned to the latest tagged release by default (`ghcr.io/ovumcy/ovumcy-web:v1.5.0`).
...
```bash
OVUMCY_IMAGE=ghcr.io/ovumcy/ovumcy-web:v1.5.0 docker compose up -d
```
